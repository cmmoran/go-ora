...
// tRawByteType reports Go types that encode as one scalar Oracle RAW value.
// Bulk array binding is explicit through NewBatch and is not inferred here.
func tRawByteType(t reflect.Type) bool {
	if t == nil {
		return false
	}
	for t.Kind() == reflect.Ptr {
